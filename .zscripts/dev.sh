...
log_step_start "Building Next.js for production (stable, low-memory)"
echo "[BUN] Building production bundle (eliminates Turbopack OOM crashes)..."
...
echo "[BUN] Starting production server (uses ~100MB instead of ~1GB dev)..."
cd "$PROJECT_DIR/.next/standalone"
node server.js > "$PROJECT_DIR/dev.log" 2>&1 &
DEV_PID=$!
cd "$PROJECT_DIR"
